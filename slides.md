...
<!-- step 1 -->
<!--
Не всё нужно тестировать. Тестовый код, как и любой код, требует поддержки. Провели чистку - надо поправить тесты. Поменяли бизнес-логику - опять тесты, добавили новую фичу - сломались тесты. Поменяли вёрстку - сломались testid для тестов. Поэтому не весь код заслуживает, чтобы его тестировали. Покрытие в 100% не гарантирует 100% защиту от регрессий, более того, это титанический труд, особенно в больших кодовых базах. Тестировать нужно наиболее важные части проекта.
...
Да, это крайности. На самом деле, конечно, *"важность"* кода это, скорее, градиент. Что-то важнее, что-то можно удалить, и никто не заметит, что-то посередине.

Давайте разделим код на условные "типы":

- [click] Бизнес логика. Часто связана с деньгами, и поэтому, ну, наверное, это важно. Даже если не приносит деньги напрямую, код решает проблему пользователя, а значит этому функционалу нельзя ломаться.
...
- [click] Связующий код. Контроллеры, фасады, адаптеры. Важно? Может и важно. Почему бы и не написать тест на метод, который используется в 20 местах. Нарушение его работы может привести к багам по всему проекту. Геттеры, сеттеры, методы-однострочники, которые просто пробрасывают данные дальше, наверное, тестировать не нужно.
...
<!--
А что вообще такое юнит тест? Это автоматизированный тест, который:
...
[click] Также есть классическая школа тестирования. На фронте или, по крайней мере, для Angular, все пишут по лондонской, которая выше. Я думаю, это связано с фундаментальными ограничениями тестовых фреймворков, ведь большинство раннеров работают в ноде, поэтому не получается заменять все зависимости.

Вроде, у всех из нас в команде были проблемы с запуском тестов, а-ля нет провайдера, ломаются код из-за другой среды исполнения и всё такое, всё приходится мокировать. Происходит ситуация, когда мы мокируем всё, кроме тестируемого класса. Это и есть лондонская школа.

По классической, мокируется минимум зависимостей, только для внепроцессных зависимостей типа API, баз данных, файловой системы и сторонних сервисов пишут заглушки. Такой подход приводит к тестам, которые покрывают значительно больше кода, лучше защищают от багов, но и у них есть свои недостатки. Я слышал, что такой подход хорошо применим к бэку, особенно в Максиме, так как у нас постоянно делят функциональность на отдельные сервисы.

[click] Я буду говорить, по большей части, о лондонской школе, которую используют во фронте, но и о бэке я не забуду.
-->
...
- Если тест падает, то можно точно понять, в каком месте, каком классе и какой конкретный функционал сломался. Это потому, что мы тестируем только один класс. Для классической школы это не обязательно, так как зависимости не мокируются, и ошибки в соседних классах приведут к провалам везде и всюду.

- Тестами легко покрыть сложный граф зависимостей, так как "реальным" остаётся только один класс. Всё остальное всегда будет работать так, как вы это определили моками.
-->
...
- Не используем моки, но всё равно изолирован от других тестов
- Работает медленно
...
Не все интеграционные тесты проверяют несколько единиц поведения. Однако все интеграционные тесты проверяют, что разные компоненты системы слаженно работают между собой, и покрывают огромное количество кода.
-->
...
[click:2] Вместо if'ов и дубликатов секций лучше напишите два разных теста. Юнит тесты происходят быстро, время прогона при этом увеличится ну на пару милисекунд. Если это не интеграционные тесты. И даже так, читаемость важна. Поэтому семь раз подумайте прежде чем склеить два теста в один.
...
[click:3] Частично ситуацию можно исправить, если назвать аргументы, но даже так читать это тяжело и не понятно, что вообще из себя представляет тест. Слишком много всего убрано, и приходится бегать глазами по всему файлу.
-->
...
<!--
Ну хорошо, а как тогда выкручиваться из ситуации, когда в тестах куча повторений? Помним, что тестовый код требует поддержки. При написании тестов наша задача написать как можно более баго-защищающие тесты с минимальной затратой на поддержку. Скажем, у нас есть вот такой тест. Проверяем, что дата доставки в прошлое некорректна.

[click] Далее нам нужно написать ещё несколько тестов, которые проверяют ту же самую функциональность, и требуют одних и тех же действий. Единственное отличие в них будет в константных значениях: -1 поменяем на 0, 1 или 999. [click] Даже по названиям видно: доставка на вчера, сегодня, завтра, через месяц и три месяца. [click] В таких случаях применяются параметризованные тесты. В любом фреймворке и языке можно сделать что-то похожее, и такие тесты отлично справляются с задачей "максимально защитить от багов с минимальной поддержкой".
...
[click:2] В итоге у нас получаются легко читаемые и понятные тесты, которые не страшно отрефакторить или изменить.
-->
...
- Если тестируете утилиты пишите просто, как если бы объясняли не-программисту
...
[click] Если следовать принципу "тестируй поведение", то этот косяк можно выявить. Для правильной работы сервиса вам потребуется протестировать сценарий "Переименовывает пользователя". [click] В секции действий должен быть один вызов, а тут их два.

[click:4] Таким образом, тести делают ваш код красивым
-->
